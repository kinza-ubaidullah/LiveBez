        ...
        <div className="premium-card p-6 md:p-12 bg-white text-slate-900 overflow-hidden relative border-t-8 border-blue-600 shadow-2xl">
            ...
            {isMatchLive && (
                <div className="absolute top-2 md:top-4 right-2 md:right-4 flex items-center gap-1.5 md:gap-2 px-2 md:px-3 py-0.5 md:py-1 bg-red-50 rounded-full border border-red-100 animate-pulse">
                    ...
                    <span className="text-[8px] md:text-[10px] font-black uppercase text-red-500">Live Show</span>
                </div>
            )}

            <div className="relative z-10 flex items-center justify-between gap-2 md:gap-8 text-center">
                ...
                <div className="flex-[1.5] space-y-2 md:space-y-4">
                    <div className="flex flex-col items-center">
                        <div className="flex items-center gap-1.5 md:gap-2 mb-2 md:mb-4">
                            ...
                            {match.minute && (
                                <div className="text-blue-600 text-[10px] md:text-sm font-black tabular-nums bg-blue-50 px-2 py-0.5 md:px-3 md:py-1 rounded-full">
                                    {match.minute}&apos;
                                </div>
                            )}
                        </div>
                        <div className="text-4xl md:text-8xl font-black tracking-tighter tabular-nums flex items-center justify-center gap-2 md:gap-8">
                            <span className="text-blue-600 drop-shadow-sm">{match.homeScore ?? '0'}</span>
                            <span className="text-xs md:text-3xl opacity-10 font-light">—</span>
                            <span className="text-blue-600 drop-shadow-sm">{match.awayScore ?? '0'}</span>
                        </div>
                    </div>
